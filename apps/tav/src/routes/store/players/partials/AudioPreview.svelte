...
<audio class="hidden" preload="none" bind:this="{player}" src="{src}"></audio>
	...
	<button
			...
			<div class="float-right -mt-1 -mr-0.5">
				<IconPlay
					class="h-7 w-7 rounded-2xl text-center
					 text-primary-400 hover:border-4 hover:border-yellow-300" />
		...
		{:else}
			<div class="float-right -mt-2 -mr-1">
				<IconPause
					class="h-8 w-8 rounded-2xl border border-4 border-yellow-300 text-primary-600 drop-shadow-md" />
			</div>
		{/if}
	</button>
{/if}
