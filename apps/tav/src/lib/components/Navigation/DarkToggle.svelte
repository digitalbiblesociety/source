...
let theme_mode = ""
onMount(async () => {
	theme_mode = window.matchMedia("(prefers-color-scheme: dark)").matches ? "dark" : "light"
})
...
<button class="sm:ml-4" on:click="{toggleDark}" aria-label="Light - Dark" >
	...
	<svg
		...
		<circle
			fill="{theme_mode === 'dark' ? '#fbbf24' : 'rgb(243 244 246)'}"
			cx="12"
			cy="12"
			r="7"
			mask="url(#mask)">
