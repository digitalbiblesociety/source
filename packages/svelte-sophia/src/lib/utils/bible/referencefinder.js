bible.ReferenceFinder = (function () {
	function createLinks(text) {
		...
		for (let i = 0, il = parts.length; i < il; i++) {
			...
			for (let j = 0, jl = subParts.length; j < jl; j++) {
				...
				// 1:1 chapter:verse
					...
					const chapter = parseInt(miniParts[0], 10)
					const verse = parseInt(miniParts[1], 10)
				...
				// 1-2 verse-verse
					...
					var miniParts = subPart.split('-')
					const verse1 = parseInt(miniParts[0], 10)
					const verse2 = parseInt(miniParts[1], 10)
