...
bible.morphology.robinson = {
	...
	getPartofSpeech(partOfSpeechKey) {
		const full = this.partsOfSpeech[partOfSpeechKey.toUpperCase()]
...
bible.morphology.OSHB = {
	format(morph) {
		...
		for (let i = 0, il = morphParts.length; i < il; i++) {
			const m = morphParts[i]
			...
			for (let j = 0, jl = partsList.length; j < jl; j++) {
